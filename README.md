...
Session creation, booking, participant cancellation, and coach cancellation use serializable transactions. Row locks and guarded updates protect balances and capacity. Serializable isolation prevents committed serialization anomalies, but callers must still retry a transaction rejected with a serialization failure; the current HTTP layer reports such a failure rather than retrying automatically. Login/logout use their existing single-statement/default-isolation paths.
...
The AI assistant, anonymous visitor account creation, full rescheduling, and attendance/check-in workflow are unfinished. Rescheduling emails are therefore also unfinished. These were left out to finish and validate the existing booking, cancellation, access, calendar, email, and scheduler paths first.
